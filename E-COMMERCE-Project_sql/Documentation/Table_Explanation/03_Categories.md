![categories-table](Tables-image/create-table-categories.png)
...
A new table named **`addresses`** is being created — this will store the shipping/billing addresses of the users. A single user can have multiple addresses (establishing a One-to-Many relationship with the `users` table).
